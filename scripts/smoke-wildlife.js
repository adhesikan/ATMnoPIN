...
async function main() {
  ...
  try {
    ...
    check('homepage nav keeps Fish Tank (/community) + CREATE MY ATM links', homeNav.text.includes('<li><a href="/community">Fish Tank</a></li>') && homeNav.text.includes('<a href="/login?next=/ai-profile-generator" class="nav-cta" data-account-cta>Create My ATM</a>') && !homeNav.text.includes('class="nav-cta">Get Featured<'));
