...
- **Hero** (Sprint 1B.4): single-column, no hero-right panel. CTAs: **Create My ATM** → `/login?next=/ai-profile-generator` (primary) and **Explore Poker Wildlife** (secondary). `section#poker-wildlife` is the only Wildlife feature near the top. `renderHeroCarousel()` and the `.hc-*` CSS/inline script remain but are unused (`<!-- HERO_CAROUSEL -->` removed; the script's `if (!track) return` guard makes it a no-op).
...
- **Nav** (duplicated across `renderLayout`, `index.html`, `shop.html`): **Create My ATM** CTA (`data-account-cta`, default `/login?next=/ai-profile-generator`) + **Sign In** (`data-account-nav`). `account-nav.js` swaps them to `/ai-profile-generator` and **My ATM** → `/account` when `/api/auth/me` says authenticated.
...
`GET /ai-profile-generator` is account-first (Sprint 1B.4): logged out/unverified → 302 `/login?next=/ai-profile-generator`; verified without @username → 302 `/account/setup?next=/ai-profile-generator`; verified + @username → `renderAIFirstProfilePage()` (or `renderPokerProfileExistsPage()` if already linked). The legacy `renderAIProfileGeneratorPage()` + `POST /request-feature` backend is kept (still creates `pending` rows) but is no longer served on GET.
- **`next` handling**: `/login` and `/account/setup` accept `?next=`, validated by `safeLocalNextPath()` (single leading `/`, no `//`, backslashes, control chars, or dot-segment tricks; re-checked after URL normalization). Invalid → ignored (default `/account`). Values reach client JS only via `inlineScriptJson()`. Helpers sit just before `handleAuthRoutes()`; routes are inside it.
